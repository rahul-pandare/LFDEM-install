...
5. Update Makefile: go to LF DEM folder/LF_DEM and edit `Makefile`. Under 'include paths' update the cholmod path and set it to `Cholmod_path = -I $(SUITESPARSE_ROOT)/include/suitesparse/` (only for M2). Next, update the name of the config file to `makeconfig = config/Makefile_config_Rahul_m1.mk` (or the updated file name).
