...
void System::setupParameters()
{
	...
	if (Interactions::has_ageing_contacts(p.contact) && control == Parameters::ControlVariable::rate) {
		if (p.dt_max > 0) {
			p.dt_max = 0.1*p.ageing_contact.memory_length/imposed_flow->getRate();
			cout << "  System::\t" << "Ageing contacts in rate-control mode: set dt_max = " << p.dt_max << std::endl;
		}
	}
}

void System::setupInteractionManager(const std::vector <struct Interactions::contact_state> &contact_states) 
{
...
void System::setupInteractionManager(const std::vector <struct Interactions::contact_state> &contact_states,
									 const std::vector <struct Interactions::AgeingState> &ageing_states) 
{
	auto pair_manager = std::make_shared<Interactions::PairManager> (np);
	std::vector< std::pair< struct Interactions::contact_state, struct Interactions::AgeingState> > contacts;
	for (unsigned i=0; i<contact_states.size(); i++) {
		contacts.push_back(std::make_pair(contact_states[i], ageing_states[i]));
	}
	interaction = std::make_shared<Interactions::StdInteractionManager>(np, 
																		pair_manager,
																		conf.get(), 
...
void System::setupConfiguration(struct base_shear_configuration config, Parameters::ControlVariable control_)
{
	...
	setupInteractionManager(config.contact_states);
}
...
{
	setupGenericConfiguration(confConvertBase2Shear(config.base, config.lees_edwards_disp), control_);
	setupInteractionManager(config.base.contact_states);
	Interactions::ActAdhesion::setupInteractions(*interaction, config.adhesion_states);
}

void System::setupConfiguration(const struct ageing_contact_configuration &config,
								Parameters::ControlVariable control_)
{
	setupGenericConfiguration(confConvertBase2Shear(config.base, config.lees_edwards_disp), control_);
	setupInteractionManager(config.base.contact_states, config.ageing_states);
}
...
void System::eventShearJamming()
{
	...
	*/
	...
	if (imposed_flow->getRate() < p.sj_shear_rate && computeMaxNAVelocity() < p.sj_velocity) {
		cnt_jamming ++;
...
void System::adjustContactModelParameters()
{
	...
	if (p.max_kn_auto_det > 0 && p.contact.kn > p.max_kn_auto_det) {
	...
	if (p.disp_tan_target != -1) {
		...
		p.contact.kt += dkt;
		if (p.min_kt_auto_det > 0 && p.contact.kt < p.min_kt_auto_det) {
			p.contact.kt = p.min_kt_auto_det;
		}
		if (p.max_kt_auto_det > 0 && p.contact.kt > p.max_kt_auto_det) {
	...
	adaptTimeStepWithVelocities();
	if (p.min_dt_auto_det > 0 && dt < p.min_dt_auto_det) {
		dt = p.min_dt_auto_det;
	}
	if (p.max_dt_auto_det > 0 && dt > p.max_dt_auto_det) {
		dt = p.max_dt_auto_det;
	}
	previous_cumulated_strain = clk.cumulated_strain;
	for (auto &inter: *interaction) {
		inter->resetContactParams(p.contact);
	}
}
