...
void StokesSolver::compute_LTRHS(vector<vec3d> &F, vector<vec3d> &T)
{
	 ...
	 */
	if (!chol_L->is_ll) {
		throw std::runtime_error(" The factorization is LDL^T. compute_LTRHS(F, T) only works for LL^T factorization.");
	}
	double alpha[] = {1, 0};
	double beta[] = {0, 0};
	int transpose = 0;
...
void StokesSolver::factorizeResistanceMatrix()
{
	...
	if (chol_c.status) {
		...
		throw std::runtime_error("Cholesky factorization failed, this should not happen with a symmetric positive definite resistance matrix.");
