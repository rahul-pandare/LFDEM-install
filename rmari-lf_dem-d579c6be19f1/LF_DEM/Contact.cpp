...
void Contact::setInteractionData(const ContactParams &p)
{
	 ...
	 * This definition comes from sphere vs half-plane geometry of contact mechanics.
	...
	mu_sliding = mu_static;
	mu_rolling = p.mu_rolling;
}

void Contact::switchToStaticSlidingFriction()
{
	sliding_state = SlidingState::nonsliding; // static friction
	mu_sliding = mu_static;
}

void Contact::switchToDynamicSlidingFriction()
{
	sliding_state = SlidingState::sliding; // static friction
	mu_sliding = mu_dynamic;
	auto supportable_tanforce = mu_sliding*normal_load;
	// adjust the sliding spring for dynamic friction law
	setTangentialForceNorm(f_spring_tan.norm(), supportable_tanforce);
}
...
void Contact::calcContactSpringForce()
{
	...
	if (sliding_state == SlidingState::cl_inactive) {
...
double Contact::getMuSliding() const
{ 
	return mu_sliding;
}
...
void Contact::frictionlaw_standard()
{
	...
	if (normal_load > 0) {
		supportable_tanforce = mu_sliding*normal_load;
		if (rolling_friction()) {
	...
	if (sq_f_tan < supportable_tanforce*supportable_tanforce) {
		switchToStaticSlidingFriction();
		sliding_mobilization = std::sqrt(sq_f_tan/supportable_tanforce);
	} else {
		switchToDynamicSlidingFriction();
		sliding_mobilization = 1;
	}
	if (rolling_friction()) {
...
void Contact::frictionlaw_criticalload()
{
    ...
    if (normal_load < 0) {
		sliding_state = SlidingState::cl_inactive; // frictionless contact
		disp_tan.reset();
		f_spring_tan.reset();
		if (rolling_friction()) {
			disp_rolling.reset();
		...
		double supportable_tanforce = mu_sliding*normal_load;
...
void Contact::frictionlaw_ft_max()
{
	...
	if (sq_f_tan > ft_max*ft_max) {
		sliding_state = SlidingState::sliding; // dynamic friction
		setTangentialForceNorm(sqrt(sq_f_tan), ft_max);
		sliding_mobilization = 0;
	} else {
		sliding_state = SlidingState::nonsliding; // static friction
		sliding_mobilization = std::sqrt(sq_f_tan/ft_max);
	}
}
