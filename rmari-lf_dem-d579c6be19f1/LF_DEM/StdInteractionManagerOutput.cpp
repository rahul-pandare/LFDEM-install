...
void output(const StdInteractionManager &manager, ParticleVelocity *vel, const struct Parameters::ParameterSet &p, OutputData &outdata_int)
{
	...
	for (const auto &inter: manager.interactions) {
		...
		if (inter->contact) {
			cs = inter->contact->getSlidingState();
			...
			mobilization = inter->contact->getSlidingFrictionMobilization();
		}
		...
		outdata_int.entryData("sliding friction mobilization", Dimensional::Dimension::none, 1, mobilization);

		if (Interactions::has_ageing_contacts(p.contact)) {
			double mu = 0;
			if (inter->contact) {
				mu = inter->contact->getMuSliding();
			}
			outdata_int.entryData("mu_sliding", Dimensional::Dimension::none, 1, mu);
		}
