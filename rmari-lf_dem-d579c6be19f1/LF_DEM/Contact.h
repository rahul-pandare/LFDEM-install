...
enum class SlidingState : unsigned {
	nocontact = 0,
	cl_inactive = 1,  // Friction is not activated (critical load model)
	nonsliding = 2,
	sliding = 3
};
...
private:
	...
	void switchToStaticSlidingFriction();
	void switchToDynamicSlidingFriction();
...
protected:
	...
	SlidingState sliding_state;

	double sliding_mobilization;
...
public:
	...
	virtual void incrementDisplacements(double dt, const struct PairVelocity &vel);
	...
	{
		return sliding_state != SlidingState::nocontact && sliding_state != SlidingState::cl_inactive;
	}
	unsigned getSlidingState() const
	{
		return static_cast<unsigned>(sliding_state);
	}
	double getSlidingFrictionMobilization() const
	{
		return sliding_mobilization;
	}
};
