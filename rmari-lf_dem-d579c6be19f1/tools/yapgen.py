...
def get_particle_yaparray(frame_par, pcols, is2d, particle_colors):
    # display a circle for every particle
    yap_out = pyp.layer_switch(3)
    yap_out = pyp.add_color_switch(yap_out, 2)
...
def get_box_yaparray(par_f_meta, is2d):
    # display bounding box
    yap_out = pyp.layer_switch(4)
    yap_out = pyp.add_color_switch(yap_out, 0)
    lx2 = float(par_f_meta['Lx'])/2
    ly2 = float(par_f_meta['Ly'])/2
    lz2 = float(par_f_meta['Lz'])/2
    if not is2d:
...
def snaps2yap(pos_fname,
    ...
    for frame_par in par_f:
        yap_out = get_particle_yaparray(frame_par, pcols, is2d, particle_colors)

        bounding_box = get_box_yaparray(par_f.meta_data(), is2d)
        yap_out = np.row_stack((yap_out, bounding_box))


        # display strain et al
        ...
        time = du.matching_uniq(frame_par[0], ["time".encode('utf8')])
        shear_disp = du.matching_uniq(frame_par[0], ["shear disp".encode('utf8')])
        ...
        yap_out = np.row_stack((yap_out,
                                ['t', 1.2*lx2, str(0.), 1.3*lz2,
                                    'cumulated strain='+str(strain[1]), '', '']))
        yap_out = np.row_stack((yap_out,
                                ['t', 1.2*lx2, str(0.), 1.2*lz2,
                                    'shear_disp='+str(shear_disp[1]), '', '']))
        yap_out = np.row_stack((yap_out,
                                ['t', 1.2*lx2, str(0.), 1.1*lz2,
                                    'time='+str(time[1]), '', '']))
