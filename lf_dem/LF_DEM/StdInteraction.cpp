...
void StdInteraction::resetContactParams(const struct ContactParams &contp)
{
	*(p.contp) = contp;
	if (contact) {
		auto cstate = contact->getState();
		struct AgeingState astate;
		if ((p.contp)->friction_model == FrictionModel::ageing_Coulomb) {
			Interactions::AgeingContact *ageing_contact =  dynamic_cast<Interactions::AgeingContact*>(contact.get());
			astate = ageing_contact->getAgeingState();
		}
		switchOffContact();
		switchOnContact();

		contact->setState(cstate);
		if ((p.contp)->friction_model == FrictionModel::ageing_Coulomb) {
			Interactions::AgeingContact *ageing_contact =  dynamic_cast<Interactions::AgeingContact*>(contact.get());
			ageing_contact->setAgeingState(astate);
		}
	}
}
