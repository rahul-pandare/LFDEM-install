...
void Simulation::stressProgram()
{
	...
	for (auto &inter: *(sys.interaction)) {
		inter->resetContactParams(sys.p.contact);
	}
	sys.calculateForces();
...
void Simulation::outputConfigurationBinary(string conf_filename)
{
	...
	if (Interactions::has_ageing_contacts(sys.p.contact)) {
		binary_conf_format = ConfFileFormat::bin_ageing_contacts;
	}
...
void Simulation::outputData()
{
	...
	double mobilization = evaluateFrictionMobilization(sys);
	outdata.entryData("average sliding friction mobilization", Dimensional::Dimension::none, 1, mobilization);
	...
	/****************************   Fabric Tensor Output *****************/
	if (sys.p.output.out_fabric) {
		outdata_fabric.entryData("contact fabric tensor (xx, xy, xz, yz, yy, zz)", Dimensional::Dimension::none, 6, evaluateFabricTensor(sys, 0.));
		outdata_fabric.entryData("fabric tensor cutoff 0.01 (xx, xy, xz, yz, yy, zz)", Dimensional::Dimension::none, 6, evaluateFabricTensor(sys, 0.01));
		outdata_fabric.writeToFile();
	}
}
...
void Simulation::getSnapshotHeader(stringstream& snapshot_header)
{
	...
	if (sys.shear_type == ShearType::simple_shear) {
		snapshot_header << "# shear disp x" << sep << sys.lees->getShearDisp().x << endl;
		snapshot_header << "# shear disp y" << sep << sys.lees->getShearDisp().y << endl;
	}
...
void Simulation::outputParFileTxt()
{
	...
	if (sys.p.output.origin_zero_flow) {
		...
		for (int i=0; i<np; i++) {
			...
			pos[i].y = sys.conf->position[i].y-0.5*sys.get_ly();
			pos[i].z = sys.conf->position[i].z-0.5*sys.get_lz();
		}
	}
	...
	for (unsigned i=0; i<sys.get_np(); i++) {
		...
		outdata_par.entryData("radius", Dimensional::Dimension::none, 1, sys.conf->radius[i]);
		if (!sys.twodimension) {
		...
		outdata_par.entryData("angular velocity (x, y, z)", Dimensional::Dimension::Velocity, 3, vel.ang_vel[i]);
		if (sys.twodimension) {
...
void Simulation::outputFinalConfiguration(const string& filename_import_positions)
{
	...
	int np = sys.get_np();
	for (int i=0; i<np; i++) {
		fout_finalconfig << sys.conf->position[i].x << ' ';
		fout_finalconfig << sys.conf->position[i].y << ' ';
		fout_finalconfig << sys.conf->position[i].z << ' ';
		fout_finalconfig << sys.conf->radius[i] << endl;
	}
	...
	if (sys.shear_type == ShearType::simple_shear) {
		for (int i=0; i<np; i++) {
			pos[i] = shiftUpCoordinate(sys.conf->position[i].x-0.5*lx,
									   ...
									   sys.conf->position[i].z-0.5*lz);
		}
	} else {
		vec3d pos_shift(0.5*lx, 0.5*ly, 0.5*lz);
		for (int i=0; i<np; i++) {
			pos[i] = sys.conf->position[i]-pos_shift;
		}
	}
	/* If the origin is shifted,
	 * we need to change the velocities of particles as well.
...
void Simulation::outputGSD()
{
	...
	//	particles/orientation
	{
		if (sys.twodimension) {
			...
			for (int i=0; i<np; i++) {
				int i4 = 4*i;
				fptr[i4  ] = cos(sys.conf->angle[i]/2);
				fptr[i4+1] = 0;
				fptr[i4+2] = 0;
