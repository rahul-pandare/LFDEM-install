...
double evaluateFrictionMobilization(const System &sys)
{
	double mobil = 0;
	unsigned total_nb = 0;
	...
	for (const auto &inter: *(sys.interaction)) {
		if (inter->contact) {
			mobil += inter->contact->getSlidingFrictionMobilization();
			total_nb++;
		}
	}
	if (total_nb) {
		return mobil/total_nb;
	} else {
		return 0;
	}
}

Sym2Tensor evaluateFabricTensor(const System &sys, double reduced_gap_cutoff = 0)
{
	unsigned total_nb = 0;
	Sym2Tensor fabric;

	for (const auto &inter: *(sys.interaction)) {
		if (inter->getReducedGap() <= reduced_gap_cutoff) {
			fabric += outer(inter->getUnitSeparation());
			total_nb++;
		}
	}
	if (total_nb) {
		return fabric/total_nb;
	} else {
		return {1, 0, 0, 0, 1, 1}; // identity
	}
}
