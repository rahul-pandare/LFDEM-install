...
void readBinaryPosition(std::ifstream &input, struct base_configuration &c)
{
	unsigned np;
...
struct base_configuration readBinaryBaseConfiguration(std::ifstream &input)
{
	...
	readBinaryPosition(input, c);
	c.contact_states = Interactions::Contact_ios::readStatesBStream(input, c.position.size());
	return c;
}
...
struct ageing_contact_configuration readBinaryAgeingContactsConfiguration(std::string filename)
{
	checkInFile(filename);
	auto format = getBinaryConfigurationFileFormat(filename);
	std::set<ConfFileFormat> allowed_formats = \
	{
		ConfFileFormat::bin_ageing_contacts,
	};
	if (!allowed_formats.count(format)) {
		throw std::runtime_error("readBinaryAgeingContactsConfiguration(): got incorrect binary format.");
	}
	std::ifstream input(filename.c_str(), std::ios::binary | std::ios::in);
	struct ageing_contact_configuration c;
	...
	readBinaryPosition(input, c.base);
	const auto &ageing_contact_states = Interactions::Contact_ios::AgeingContact::readStatesBStream(input, c.base.position.size());
	for (const auto &acs: ageing_contact_states) {
		c.base.contact_states.push_back(acs.first);
		c.ageing_states.push_back(acs.second);
	}
	c.lees_edwards_disp.reset();
	input.read((char*)&c.lees_edwards_disp.x, sizeof(double));
	input.read((char*)&c.lees_edwards_disp.y, sizeof(double));
							   ...
							   ConfFileFormat format)
{
	...
	if (format == ConfFileFormat::bin_ageing_contacts) {
		Interactions::Contact_ios::AgeingContact::writeStatesBStream(conf_export, *(sys.interaction));
	} else {
		Interactions::Contact_ios::writeStatesBStream(conf_export, sys.getBaseConfiguration().contact_states);
	}
